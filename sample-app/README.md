...
A deterministic Flutter e-commerce **fixture** for the mobile-automator CLI. It is **not** a user-facing product — it exists so contributors and CI can drive `mauto` end-to-end against a stable target. See PRD: https://github.com/sh3lan93/mobile-automator/issues/44

## Why it exists

The CLI had zero automated coverage of the real mobile-mcp execution path. This app is the known-good target that later slices (#46–#49) drive in CI.
...
## For mauto contributors

Point `mauto` at this directory as a test workspace, or drive it in CI (slice #46+). The app is deterministic: bundled JSON, no network, cart/favorites reset on cold start.
