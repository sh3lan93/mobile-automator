...
- **Commander parse errors now flow through the JSON envelope instead of bypassing it.** `mauto result add-step --run-id X --screenshot shot.png` used to print `error: unknown option '--screenshot'` as bare stderr text and exit 1 — commander detected the bad flag before any action callback ran, so the `{ok,data,error,hint,schema_version}` contract every host agent parses simply never emitted, and a malformed invocation was indistinguishable from a crashed device. `buildProgram()` now sets `exitOverride()` + `configureOutput({writeErr})` on the root program (inherited by every subcommand), and `toEnvelope()` classifies **any** `CommanderError` as `invalid_input` (exit 3) — with commander's message preserved in `error` and the failing command's usage line in `hint` — except the closed set of display outcomes (`help`, `helpDisplayed`, `version`), which `run()` short-circuits to commander's own exit code. The classification is a denylist rather than an allowlist of failure codes on purpose: commander v12 defines 13 codes, the set grows with each release, and anything unlisted would fall through to `internal` — the panic bucket, which maps to exit 1 and makes `diagnose()` dump a stack trace. That is not hypothetical: `mauto result add-step --run-id` (flag supplied, value omitted — `optionMissingArgument`, a near-synonym of the handled `missingMandatoryOptionValue`) emitted `kind:"internal"`, exit 1, and a 10-frame stack trace, across all 60 value-taking options in the CLI. `mauto --help` / `mauto --version` keep their human-readable output and exit 0, and `--human` renders parse failures readably. A drift guard scans commander's own source for its error-code surface and fails if any code reaches `internal`, so a future commander upgrade cannot silently reopen the gap. ([#146](https://github.com/sh3lan93/mobile-automator/issues/146))
- **Unset placeholders no longer interpolate inside code spans.** The not-configured fallback for `{{build_command}}` contained backticks of its own, and four guide lines placed the token inside a markdown code span — so on an unconfigured workspace (exactly a new user's first contact) the emitted guide rendered nested-backtick soup at the spot meant to show a command, and on two lines presented the fallback string itself as the command to run. The root cause was not the code span but the fallback: one context-blind string was substituted into 13 placeholder sites spanning five grammatical roles (label value, inline noun, parenthetical appositive, inline tail, whole section body), and no single string fits all of them. Because the fallback parenthesized itself, every appositive slot rendered doubled parens — `the app package ((not configured …))` — which affected the `{{app_package}}` and `{{loading_indicators}}` lines too, not just `{{build_command}}`. Unset placeholders now render per slot kind: **value** slots get a bare, unparenthesized note that names the key to set (`not configured — mauto config set build_command`, where the old text left `<key>` literal and never said which key was missing), and **optional** slots (`{{automation_extras}}`, `{{additional_resources}}`) render as nothing, so `**Automation:** the `mauto` CLI.` ends cleanly instead of having a note jammed onto it. The guard moved to the layer that owns the invariant: `tests/lint/guide-no-placeholder-leak.test.js` already emits every topic × mode with no config — precisely the unconfigured first-contact state — so it now also asserts no doubled parens, no fallback inside a code span, and no fallback jammed against the preceding word. A guard that scans the raw content files cannot see any of this, since every malformation exists only after interpolation. ([#143](https://github.com/sh3lan93/mobile-automator/issues/143))
...
- `mauto --version` / `-V` prints the installed package version and exits 0, via commander's own `.version()` on the root program. It is deliberately **not** a hand-rolled `argv` pre-scan: a scan such as `argv.some((a) => a === '--version')` is positionally blind, and in particular cannot honor the POSIX `--` separator — which is a state transition in the parser (everything after it is an operand), not a token that can be matched. A scan therefore hijacks the flag even when it is supplied as a value, so `mauto type -- --version` printed the version and exited 0 instead of typing the text, and no invocation at all could pass a literal `--version` / `-V` to `type`, `memory add`, `config set`, or `screenshot`. Commander tracks that state, so `--` works again and the flag now also appears in `mauto --help`, where the hand-rolled version was invisible.
