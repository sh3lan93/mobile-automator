...
- **The support floor moves to Node 20, now with a single mechanically-checked source of truth instead of eight independent claims.** `engines.node` becomes `>=20.0.0` and the matrix drops 18 to match: keeping a floor at a runtime that has been EOL since April 2025 means committing to fix any Node-18-only failure the new matrix turns up, which is maintenance spent on a version nobody should be on. `0.x` is the cheapest moment to move a floor, and npm only warns (`EBADENGINE`) for anyone below it. The range had been restated in seven prose sites across five files — the badge, the README prerequisite, both contributing guides, `docs/getting-started/index.md`, and `docs/getting-started/installation.md` twice (prerequisite and troubleshooting hint) — plus `engines.node` itself, eight literal locations free to drift from each other. `tests/lint/node-version-agreement.test.js` now derives the floor from `engines.node` and fails the build if any Node claim in a shipping doc, the matrix minimum, or a literal `node-version` in any other workflow disagrees, so the next floor change is a one-line edit that mechanically tells you every other site is stale. It matches *any* Node-adjacent major rather than an enumerated list of phrasings — an enumeration only protects the claim sites that already exist, and both `Install Node 18 first` and `Node.js 18 LTS or above` walked straight past the first draft of it. ([#162](https://github.com/sh3lan93/mobile-automator/issues/162))
- **The prose-scanning lint guards share one doc corpus.** Three guards each walked the docs tree with their own hand-maintained exclusion list, and the three lists had already diverged: two still excluded a `docs/superpowers/` tree that no longer exists, and none of them looked at `sample-app/`, where `README.md` was still teaching the Gemini extension's removed `/mobile-automator:*` slash-command syntax months after the extension was excised. `tests/lint/docs-corpus.js` now enumerates the corpus once with `git ls-files` — every `.md` git knows about, minus historical records — so `.gitignore` never has to be restated in a test and coverage is a property of the repo rather than of whoever last edited a `collectDocs()`. Adopting it immediately caught two stale claims no guard could previously see: that `sample-app/README.md`, and a Node 18 environment in `CONTRIBUTING.md`'s sample bug report. `package-lock.json`'s agreement with `package.json` moved to its own `tests/lint/lockfile-in-sync.test.js` (it is a derived artifact, not a claim about what users need), and `lint:guides` became `jest tests/lint` — the hand-listed form had silently omitted four guards, including `action-coverage`. ([#162](https://github.com/sh3lan93/mobile-automator/issues/162))
