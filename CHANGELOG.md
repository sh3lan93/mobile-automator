...
- **Recorder — GUI URL fallback printed from `record.toml`** ([#66](https://github.com/sh3lan93/mobile-automator/issues/66)). `commands/mobile-automator/record.toml § 2.0` was redesigned to spawn the sidecar as a **background** subprocess so the URL print can fire while the sidecar is still alive. Step 2 now uses a non-blocking shell primitive (`run_in_background: true` or equivalent) and tails the sidecar's stdout/stderr via a streaming/monitor primitive; step 3 polls the `mobile-automator/.recorder/<scenario>/recorder.port` file the HTTP server writes (250ms × 12 = 3s retry to absorb the spawn-vs-bind race) and prints `🌐 Recorder GUI: http://127.0.0.1:<port>/` with a one-line hint to paste the URL into a browser; step 4 awaits the background subprocess's completion via task notification before checking the exit code. On port-file timeout the agent now issues a ⚠️ warning and continues to step 4 instead of HALTing — a transient bind delay should not abort an otherwise-healthy recording; a genuinely broken sidecar still surfaces as a non-zero exit code in step 5. Gives users a paste-able fallback for SSH sessions, CI/headless runs, `--no-gui`, and reopening a closed GUI tab within the 60s reconnect window — fully complementary to the sidecar-side `console.log` introduced in [#65](https://github.com/sh3lan93/mobile-automator/issues/65)/[PR #68](https://github.com/sh3lan93/mobile-automator/pull/68).
