...
- If that install fails because a different build of the app is already on the device, uninstall it — run `mauto uninstall` with the app package ({{app_package}}) — then install again, and report that app data was wiped.
...
3. **Always include `"$schema_version": "2.1"` as the first field in the JSON.**
